...
{
  for (i, m_el) in mat.iter_mut().enumerate().take(K) {
    for j in 0..L {
      poly_uniform(&mut m_el.vec[j], rho, ((i << 8) + j) as u16);
    }
  }
}
...
{
  for (i, m_el) in mat.iter().enumerate().take(K) {
    polyvecl_pointwise_acc_montgomery(&mut t.vec[i], m_el, v);
  }
}
