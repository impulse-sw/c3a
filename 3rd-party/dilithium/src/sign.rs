use crate::{SignError, fips202::*, packing::*, params::*, poly::*, polyvec::*, randombytes::*};

pub fn crypto_sign_keypair(pk: &mut [u8], sk: &mut [u8], seed: Option<&[u8]>) -> u8
{
...
pub fn crypto_sign_verify(sig: &[u8], m: &[u8], pk: &[u8]) -> Result<(), SignError>
{
  ...
  if c != c2 { Err(SignError::Verify) } else { Ok(()) }
}
