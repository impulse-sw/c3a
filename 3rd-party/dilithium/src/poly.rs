use crate::{fips202::*, ntt::*, params::*, reduce::*, rounding::*, symmetric::*};
...
const POLY_UNIFORM_NBLOCKS: usize = (768 + STREAM128_BLOCKBYTES - 1).div_ceil(STREAM128_BLOCKBYTES);
...
pub fn poly_uniform(a: &mut Poly, seed: &[u8], nonce: u16)
{
  ...
  let mut ctr = rej_uniform(&mut a.coeffs, N_U32, &buf, buflen);
  let mut off;
  while ctr < N_U32 {
    off = buflen % 3;
    for i in 0..off {
    ...
    ctr += rej_uniform(&mut a.coeffs[(ctr as usize)..], N_U32 - ctr, &buf, buflen);
  }
}
...
const POLY_UNIFORM_ETA_NBLOCKS: usize = if ETA == 2 {
  (136 + STREAM256_BLOCKBYTES - 1).div_ceil(STREAM256_BLOCKBYTES)
} else {
  (227 + STREAM256_BLOCKBYTES - 1).div_ceil(STREAM256_BLOCKBYTES)
};
...
const POLY_UNIFORM_GAMMA1_NBLOCKS: usize =
  (POLYZ_PACKEDBYTES + STREAM256_BLOCKBYTES - 1).div_ceil(STREAM256_BLOCKBYTES);
...
pub fn poly_challenge(c: &mut Poly, seed: &[u8])
{
  ...
  for (i, el) in buf.iter().enumerate().take(8) {
    _signs |= (*el as u64) << (8 * i);
  }
  let mut pos: usize = 8;
  // let mut b = buf[pos];
  let mut b;
...
pub fn polyeta_pack(r: &mut [u8], a: &Poly)
{
  ...
  if ETA == 2 {
    ...
    for (i, el) in r.iter_mut().enumerate().take(N / 2) {
...
pub fn polyeta_unpack(r: &mut Poly, a: &[u8])
{
  if ETA == 2 {
    for i in 0..N / 8 {
      ...
      r.coeffs[8 * i + 2] = (((a[3 * i] >> 6) | (a[3 * i + 1] << 2)) & 0x07) as i32;
    ...
    for (i, el) in a.iter().enumerate().take(N / 2) {
      r.coeffs[2 * i] = (*el & 0x0F) as i32;
      r.coeffs[2 * i + 1] = (*el >> 4) as i32;
...
{
  for i in 0..N / 4 {
    r.coeffs[4 * i] = (((a[5 * i]) as u32 | (a[5 * i + 1] as u32) << 8) & 0x3FF) as i32;
...
pub fn polyw1_pack(r: &mut [u8], a: &Poly)
{
  if GAMMA2 == (Q - 1) / 88 {
    ...
    for (i, el) in r.iter_mut().enumerate().take(N / 2) {
      *el = (a.coeffs[2 * i] | (a.coeffs[2 * i + 1] << 4)) as u8;
    }
  }
}
