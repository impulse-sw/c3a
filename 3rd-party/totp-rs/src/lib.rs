...
impl TOTP {
  ...
  pub fn sign(&self, time: u64) -> Vec<u8> {
    self
      .algorithm
      .sign(self.secret.as_ref(), (time / self.step).to_be_bytes().as_ref())
  }
  ...
  pub fn generate(&self, time: u64) -> String {
    ...
    #[allow(unused_mut)]
    let mut result = u32::from_be_bytes(result[offset..offset + 4].try_into().unwrap()) & 0x7fff_ffff;
    ...
    match self.algorithm {
      ...
      Algorithm::Steam => (0..self.digits)
        .map(|_| {
          let c = STEAM_CHARS.chars().nth(result as usize % STEAM_CHARS.len()).unwrap();
  ...
  pub fn get_secret_base32(&self) -> String {
    base32::encode(base32::Alphabet::RFC4648 { padding: false }, self.secret.as_ref())
  }
  ...
  pub fn from_url<S: AsRef<str>>(url: S) -> Result<TOTP, TotpUrlError> {
    let (algorithm, digits, skew, step, secret, issuer, account_name) = Self::parts_from_url(url)?;
...
mod tests {
  ...
  fn comparison_different_algo() {
    let reference = TOTP::new(Algorithm::SHA1, 6, 1, 1, "TestSecretSuperSecret".into()).unwrap();
  ...
  fn checks_token_with_skew() {
    let totp = TOTP::new(Algorithm::SHA1, 6, 1, 1, "TestSecretSuperSecret".into()).unwrap();
    assert!(totp.check("174269", 1000) && totp.check("659761", 1000) && totp.check("260393", 1000));
  }
  ...
  fn from_url_err() {
    ...
    assert!(TOTP::from_url("otpauth://totp/GitHub:test:?secret=ABC&digits=8&period=60&algorithm=SHA256").is_err());
  ...
  #[cfg(feature = "otpauth")]
  fn from_url_default() {
    let totp = TOTP::from_url("otpauth://totp/GitHub:test?secret=KRSXG5CTMVRXEZLUKN2XAZLSKNSWG4TFOQ").unwrap();
    assert_eq!(
      totp.secret,
      base32::decode(
  ...
  fn from_url_query() {
    let totp = TOTP::from_url(
      "otpauth://totp/GitHub:test?secret=KRSXG5CTMVRXEZLUKN2XAZLSKNSWG4TFOQ&digits=8&period=60&algorithm=SHA256",
    )
    .unwrap();
    assert_eq!(
      totp.secret,
  ...
  fn from_url_query_sha512() {
    let totp = TOTP::from_url(
      "otpauth://totp/GitHub:test?secret=KRSXG5CTMVRXEZLUKN2XAZLSKNSWG4TFOQ&digits=8&period=60&algorithm=SHA512",
    )
    .unwrap();
    assert_eq!(
      totp.secret,
