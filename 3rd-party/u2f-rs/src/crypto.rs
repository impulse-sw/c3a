...
impl<'a> X509PublicKey<'a> {
  ...
  pub(crate) fn is_secp256r1(&self) -> Result<bool, U2fError> {
    ...
    for i in 0..self.cert_der.len() {
      if i + secp256r1_oid.len() <= self.cert_der.len() && self.cert_der[i..i + secp256r1_oid.len()] == secp256r1_oid {
        return Ok(true);
      }
    }
...
fn extract_ec_public_key_bytes(cert_der: &[u8]) -> Option<&[u8]> {
  ...
  for i in 0..cert_der.len() - ec_pubkey_oid.len() {
    if cert_der[i..i + ec_pubkey_oid.len()] == ec_pubkey_oid {
      ...
      for j in i + ec_pubkey_oid.len()..std::cmp::min(i + ec_pubkey_oid.len() + 20, cert_der.len() - p256_oid.len()) {
      ...
      if p256_found {
        ...
        for j in i + ec_pubkey_oid.len()..std::cmp::min(i + 100, cert_der.len() - 3) {
          if cert_der[j] == 0x03 {
            ...
            if cert_der[j + offset] == 0x00 && j + offset + 1 < cert_der.len() && cert_der[j + offset + 1] == 0x04 {
...
mod tests {
  ...
  fn test_01_generate_test_files() {
    ...
    if !ec_key_path.exists() && openssl_available {
      ...
      fs::copy(&cert_der_path, test_dir.join("test_cert.der")).expect("Failed to copy certificate");
      ...
      fs::copy(&signature_path, test_dir.join("valid_p256_signature.bin")).expect("Failed to copy signature");

      // Generate additional test certificates
      let cert_types = [
        ("cert_p256.pem", "req -new -x509 -key", &ec_key_path, "/CN=example.com"),
        (
          "cert_not_p256.pem",
  ...
  fn create_dummy_test_files(test_dir: &Path) {
    ...
    let sample_p256_key = [
      0x04, 0x60, 0xf7, 0xf8, 0x5b, 0x4b, 0x72, 0xac, 0x6a, 0x24, 0x02, 0x87, 0x5c, 0x4a, 0xe9, 0x32, 0xc3, 0x9f, 0x16,
      0xaf, 0x54, 0xc3, 0xb4, 0x56, 0xf5, 0xb5, 0xea, 0x0a, 0x7c, 0xe8, 0xf7, 0x87, 0x82, 0xe5, 0xca, 0x8d, 0x4c, 0xe9,
      0x42, 0x2b, 0x35, 0x0a, 0x0d, 0x2e, 0xf8, 0x96, 0x01, 0x81, 0xba, 0x14, 0x8f, 0xd0, 0x5b, 0x35, 0x39, 0x08, 0x4a,
      0xc5, 0xd7, 0x35, 0x80, 0x74, 0x2a, 0xce,
    ];

    // Write the raw public key
    fs::write(test_dir.join("test_pub_raw.bin"), &sample_p256_key).expect("Failed to write sample public key");

    // Sample DER certificate with P-256 key
    let sample_cert_p256 = include_bytes!("../tests/data/cert_p256.der");
    fs::write(test_dir.join("cert_p256.der"), sample_cert_p256).expect("Failed to write sample P-256 certificate");

    // Sample DER certificate with RSA key
    let sample_cert_rsa = include_bytes!("../tests/data/cert_not_p256.der");
    fs::write(test_dir.join("cert_not_p256.der"), sample_cert_rsa).expect("Failed to write sample RSA certificate");
    ...
    fs::write(test_dir.join("valid_cert.der"), sample_cert_p256).expect("Failed to write sample valid certificate");
    fs::write(test_dir.join("test_cert.der"), sample_cert_p256).expect("Failed to write sample test certificate");
    ...
    sample_signature.extend(vec![0x01; 32]);
    fs::write(test_dir.join("valid_signature.bin"), &sample_signature).expect("Failed to write sample signature");
  ...
  fn extract_raw_pubkey_from_der(der_path: &Path, output_path: &Path) {
    ...
    file.read_to_end(&mut der_data).expect("Failed to read DER data");
  ...
  fn read_test_file(filename: &str) -> Vec<u8> {
    ...
    if path.exists() {
      ...
      if fallback_path.exists() {
        let mut file = File::open(&fallback_path).expect("Failed to open fallback test file");
        let mut data = Vec::new();
        file.read_to_end(&mut data).expect("Failed to read fallback test file");
        data
      } else {
        panic!(
  ...
  fn test_nistp256key_from_bytes() {
    ...
    println!("Key bytes prefix: {:02X?}", &valid_point[0..min(5, valid_point.len())]);
  ...
  fn test_extract_ec_public_key_bytes() {
    ...
    assert!(key_bytes.is_none(), "Should NOT extract EC key from non-EC certificate");
  }
